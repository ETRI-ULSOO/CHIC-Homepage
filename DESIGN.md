...
### 도해 — 이 시스템에서 가장 중요한 요소

**Role:** 연구 다이어그램의 표준 제시 방식. **전 7점이 HTML/CSS다** (2026-08-05 전환 완료).
`public/images/diagrams/`는 더 이상 존재하지 않는다.

#### 왜 SVG를 버렸나

1280px 좌표계 도해를 컨테이너 폭에 맞춰 축소하면 라벨이 같은 배율로 함께 줄어든다.
실측: 본문 폭 1092px(배율 0.85)에서 **8.96px**, 모바일 325px(배율 0.26)에서 **2.67px**.
가로 스크롤로 판독은 살렸으나 한 번에 보이는 범위가 **25.5%**로 떨어져 '한눈에 보는'
도해의 본래 가치를 잃었다.

> 2026-08-03 서술("본문 컬럼 폭 ~730px에서 6px급")은 **틀린 기록이었다.** 도해는
> `.measure`(689px) 안이 아니라 `.wrap`(1092px)에 놓인다. 실측으로 교정한다.

그와 별개로 더 큰 문제가 있었다. `<img>`로 참조된 SVG의 내부 텍스트는 **페이지에
존재하지 않는다** — `innerText`에 없고 `svg text` 노드 접근도 0개다. 즉 Ctrl+F로 못 찾고,
스크린리더는 `alt` 한 줄만 읽고, 검색엔진도 못 읽는다. 종전 도해에 갇혀 있던 텍스트
노드는 **341개**였다 (실측 확인).

#### 원칙 — 축소하지 않는다. 재배치한다.

- 넓으면 단계가 **가로로 흐르고**, 좁으면 **세로로 쌓이며** 화살표가 ↓ 로 돈다.
  글자 크기는 어느 폭에서도 본문과 같다
- 화살표는 글꼴 문자가 아니라 **CSS 보더로 그린다** — 어떤 환경에서도 같은 모양이 나온다
- 박스 배경 `--c-canvas`, 캔버스 `--c-surface`. 강조 박스만 제목이 Mint
- 그리드 최소 트랙은 반드시 `minmax(min(Npx, 100%), 1fr)` — 맨 px는 320px에서 넘친다
- 범주는 **색이 아니라 위치와 라벨로 구분**한다. 악센트는 Mint 하나뿐 (파스텔 다색 금지)

#### 세 가지 구조

| 컴포넌트 | 쓰임 | 예 |
|---|---|---|
| `DiagramStages` | 단계가 화살표로 이어지는 흐름. `flow: row\|column`, `band`(나열), `note`(관통 한 줄) | 과제 개요 · 필요성 · 전시 프로세스 · 플랫폼 · 애셋 |
| `DiagramCompare` | **행마다 좌우가 짝**인 대조. 좁은 화면에서 행 단위를 유지한 채 행 안에서만 접는다 — 단순히 세로로 쌓으면 짝이 흩어진다 | 차별화 전략 |
| `ConsortiumChart` | 계층 트리 | 추진체계 |

모두 `DiagramShell`(제목 바 · 캡션 · 푸터)로 감싼다. 캡션 접두는 언어별로 다르다
(`圖 N.` / `Fig. N.`). 보더 1px `--c-border`, radius 14px, **그림자 없음**.

#### 문안은 한 벌만 둔다

- 흐름형 → `src/data/diagrams.yaml` (국·영문 한 벌)
- 대조형 → `src/data/comparisons.yaml`
- **추진체계 도해는 문안을 갖지 않는다** — `consortium.yaml`의 `role`에서 직접 생성하므로
  기관이 바뀌면 목록과 도해가 함께 바뀐다
- Concept 각 항목의 `key`가 `concept-<key>` 이름으로 문안을 가리킨다.
  `concept.yaml`은 절 제목과 설명문만 갖는다

종전에는 같은 문안이 국문 SVG·영문 SVG에 각각 박혀 있었고, 기관명은 목록까지 합쳐 3벌이었다.
