...
**Role:** 연구 다이어그램의 표준 제시 방식. 지금은 두 갈래가 공존한다.

| | 랜딩 3점 | Concept 4점 |
|---|---|---|
| 형식 | **HTML/CSS** (`components/diagram/`) | SVG + `DiagramFigure` |
| 문안 | `diagrams.yaml` · `consortium.yaml` 한 벌 | 국·영문 SVG 2벌 |
| 좁은 폭 | 세로로 재배치 | 가로 스크롤 |
| 검색·스크린리더 | **노출됨** | 노출 안 됨 |

#### 원칙 — 축소하지 않는다 (2026-08-05)

1280px 좌표계 도해를 컨테이너 폭에 맞춰 축소하면 라벨이 같은 배율로 함께 줄어든다.
실측: 본문 폭 1092px(배율 0.85)에서 **8.96px**, 모바일 325px(배율 0.26)에서 **2.67px**.
즉 판독 불가의 원인은 "도해가 작다"가 아니라 **원본을 컨테이너에 우겨넣는다**는 데 있다.
...
축소 대신 **재배치**한다. 넓으면 단계가 가로로 흐르고, 좁으면 세로로 쌓이며 화살표가 ↓ 로 돈다.
글자 크기는 어느 폭에서도 본문과 같다.

- 문안은 `src/data/diagrams.yaml`에 국·영문 한 벌로 둔다. **추진체계 도해는 문안을 갖지 않는다** —
  `consortium.yaml`의 `role`에서 직접 생성하므로 기관이 바뀌면 목록과 도해가 함께 바뀐다
...
**얻은 것**: 도해 안의 문구가 스크린리더·브라우저 검색·검색엔진에 닿는다. `<img>` 속 SVG는
셋 다 불가능했고, 이는 종전 도해에 갇혀 있던 341개 텍스트 노드 전부에 해당했다 (실측 확인).

#### SVG 도해 (Concept 4점) — 남은 갈래

- 시스템 팔레트로 재제작한 SVG. 배경 `var(--c-surface)`
- 파스텔 3색(청·주황·녹)은 단일 악센트 원칙에 어긋난다. **범주는 색이 아니라 위치와 라벨로 구분**하고 악센트는 Mint 하나만 쓴다
- SVG 내부에도 토큰 값만 사용한다. 텍스트 4.5:1, 화살표 등 의미 있는 도형 3:1 (임의 회색 금지 — 실측 필수)
- 폭이 모자라면 줄이는 대신 **가로로 스크롤**한다 (`min-width` = 원본 폭). 모바일 실측 10.50px
- 1200px 이상에서는 본문 컬럼 밖으로 빼 원본보다 크게 띄운다 (최대 1440px). 실측 11.27px
- 확대(라이트박스)는 **반드시 인라인보다 커야 한다.** 종전 규칙(`width:auto` + `max-width:96vw`)은
  모바일에서 1.11배(325→360px)에 그쳐 확대가 사실상 무동작이었다. 현재 `max(96vw, 1600px)`
- 커서 `zoom-in`, 우하단 확대 아이콘 — 확대 가능함이 보이지 않으면 아무도 클릭하지 않는다
