...
**미해결 (M-18):** 삭제해도 `d8e29e2` 커밋에 파일이 남아 있어 **git 히스토리에서는 여전히 접근 가능하다.** 완전 제거하려면 히스토리 재작성(`git filter-repo` 등) 후 force push가 필요하며, 공개 저장소이므로 이미 클론·캐시된 사본까지는 회수할 수 없다. 사용자 판단 필요.
